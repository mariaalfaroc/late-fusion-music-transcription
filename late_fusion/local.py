...
def evaluate_multimodal_transcription(
    omr_wg_files, amt_wg_files, y_true_acc_dict, match=2, mismatch=-1, gap_penalty=-1
):
    y_true_acc = []
    y_pred_comb_acc = []
    ...
    for (id_r, r), (id_r_prob, r_prob), (id_q, q), (id_q_prob, q_prob) in zip(
        omr_y_pred_acc.items(),
        omr_y_pred_prob_acc.items(),
        amt_y_pred_acc.items(),
        amt_y_pred_prob_acc.items(),
    ):
...
def k_fold_multimodal_experiment(match=[2], mismatch=[-1], gap_penalty=[-1]):
    ...
    print(
        "k-fold multimodal image and audio music transcription combining kaldi word graphs following an Smith-Waterman policy experiment"
    )
    ...
    for i in range(k):
        ...
        omr_wg_fnames = sorted(
            [
                os.path.join(config.output_dir / "omr" / f"Fold{i}" / "SW", fname)
                for fname in os.listdir(config.output_dir / "omr" / f"Fold{i}" / "SW")
            ]
        )
        amt_wg_fnames = sorted(
            [
                os.path.join(config.output_dir / "amt" / f"Fold{i}" / "SW", fname)
                for fname in os.listdir(config.output_dir / "amt" / f"Fold{i}" / "SW")
            ]
        )
        ...
        for m, mism, gp in zip(match, mismatch, gap_penalty):
            ...
            symer, seqer = evaluate_multimodal_transcription(
                omr_wg_files=omr_wg_fnames,
                amt_wg_files=amt_wg_fnames,
                y_true_acc_dict=gt,
                match=m,
                mismatch=mism,
                gap_penalty=gp,
