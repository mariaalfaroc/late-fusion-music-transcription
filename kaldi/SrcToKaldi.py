...
def read_ConfMat(args):
    ...
    if not os.path.exists(os.path.join(args.greedy, "Results")):
        os.makedirs(os.path.join(args.greedy, "Results"))
    ...
    with open(args.src) as infile:
        for line in infile:
            ...
            if "inf" in line:
                line = process_inf(line)
            fout.write(line)
            if "[" not in line and "]" not in line:
                greedy.append(np.argmax([float(u) for u in line.split()]))
            if "]" in line:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dst", type=str, required=True, help="Destination path for the files"
    )
    parser.add_argument(
        "--greedy",
        type=str,
        required=False,
        help="Destination path for the greedy decoding",
    )
    ...
    if not os.path.exists(os.path.join(os.path.split(args.greedy)[0], "grnTruth.dat")):
        shutil.copy(
            os.path.join(os.path.split(args.src)[0], "grnTruth.dat"),
            os.path.join(os.path.split(args.greedy)[0], "grnTruth.dat"),
        )
